...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
        //producing the effect of random error which sets the number of bytes received as 0
        if(randerror%7==0)
	{
               recv_result = 0;//setting the received result as 0 to fake the effect of packet loss
	...
	wait(5);
       ...
       if(recv_result != 0 && recv_frame.frame_kind == 1 && recv_frame.sq_no==frame_id && recv_frame.checksum == csum)
   	 ...
   	 //when the checksum condition or recv result =0 the timer expires and no ack is sent and frame id is decremented
    	else if(recv_result <= 0 || csum == 0)
        {
            printf("Frame %d time expired\n", frame_id);
	    frame_id--; //frame id is being decremented
            //break;
        }
        else
        {
